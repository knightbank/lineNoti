...
//const token = '1KlQEQKKCBGFearZuNcjkyCYVzHm39vQLojHVeJC3IZ';
const token = 'dQGpdkriRH6nsJdVeJ5gVS4p5yNK6TL03I0M8JhZKF2';

//Notified flag 
var notified = false;
//notify every x milliseconds
var notiInterval=180000;
//tickker check every x milliseconds
var tickerInterval=10000;


function SendNotify(text,stickerPkg,stickerId,forceNotify=false) {
	if(notified&&!forceNotify){
		return;
	}
	var msg = text;
	request({
...
setInterval(getTicker,tickerInterval,'btcusd');
//setInterval(getTicker,tickerInterval,'ethusd');
setInterval(updateNotifyFlag,notiInterval,false);

function updateNotifyFlag(flg) {
	notified=flg;
}
